...
export function LevelSelectModal({
  ...
  return (
    <Modal open={open}>
      <Map size={54} className="block mb-[2px] mx-auto text-[#9b7fd4]" />
      <h2 className="text-[22px] font-extrabold text-[#4a3728] mt-[4px] mb-[6px]">Pilih Level</h2>
      ...
      <div className="grid grid-cols-3 gap-[10px] mt-[10px] mb-[4px]">
        {levels.map((lv: LevelData, i: number) => {
          ...
          return (
            <button
              key={i}
              disabled={locked}
              className={`border-none rounded-[16px] cursor-pointer py-3 px-[6px] flex flex-col items-center gap-[4px] font-extrabold text-[#4a3728] shadow-[0_4px_0_rgba(0,0,0,0.15)] active:translate-y-[3px] active:shadow-[0_1px_0_rgba(0,0,0,0.15)] transition-opacity ${locked ? "opacity-50 cursor-not-allowed bg-[#e2e2e2]" : "bg-gradient-to-b from-white to-[#f2e4c6]"}`}
              onClick={() => onSelect(i)}
            >
              ...
              <span className="text-[11px] text-center leading-tight w-full break-words line-clamp-2">
                {lv.name}
              </span>
